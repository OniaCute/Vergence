...
public class MemoryIndicator extends Module {
    public static MemoryIndicator INSTANCE;
    private boolean lastMouseStatus = false;
    private double lastMouseX = -1;
    private double lastMouseY = -1;

    public MemoryIndicator() {
        super("MemoryIndicator", Category.HUD);
        INSTANCE = this;
        setX(0);
        setY(0);
        setWidth(0);
    ...
    public Option<String> text = addOption(new TextOption("Text", "{memory} / {memory_max} MEM"));
    ...
    public void onDraw2D(DrawContext context, float tickDelta) {
        ...
        setWidth(FontUtil.getWidth(size, iconText) + FontUtil.getWidth(size, text.getValue()) + 3 * 2 + 3); // 3 padding
        ...
        FontUtil.drawText(
                context,
                text.getValue(),
                getX() + 5 + FontUtil.getWidth(size, iconText),
