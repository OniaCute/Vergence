...
public class TotemAnimation extends Module {
    ...
    public TotemAnimation() {
        super("TotemAnimation", Category.VISUAL);
        INSTANCE = this;
    }

    public Option<Enum<?>> mode = addOption(new EnumOption("Mode", Modes.FadeOut));
    public Option<Double> speed = addOption(new DoubleOption("Speed", 1 ,100, 40));
    ...
    public void renderFloatingItem(float tickDelta) {
        if (floatingItem != null && floatingItemTimeLeft > 0 && !mode.getValue().equals(Modes.Off)) {
    ...
    private int getTime() {
        int invertedSpeed = 101 - speed.getValue().intValue();

        if (mode.getValue().equals(Modes.FadeOut)) {
            return invertedSpeed / 4;
        }

        if (mode.getValue().equals(Modes.Insert)) {
