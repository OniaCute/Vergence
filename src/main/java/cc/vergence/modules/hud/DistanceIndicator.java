...
public class DistanceIndicator extends Module {
    public static DistanceIndicator INSTANCE;
    private boolean lastMouseStatus = false;
    private double lastMouseX = -1;
    private double lastMouseY = -1;

    public DistanceIndicator() {
        super("DistanceIndicator", Category.HUD);
        INSTANCE = this;
        setX(0);
        setY(0);
        setWidth(0);
    ...
    public Option<String> text = addOption(new TextOption("Text", "{combat_distance} m"));
    ...
    public void onDraw2D(DrawContext context, float tickDelta) {
        ...
        setWidth(FontUtil.getWidth(size, iconText) + FontUtil.getWidth(size, text.getValue()) + 3 * 2 + 4); // 3 padding
        ...
        FontUtil.drawText(
                context,
                text.getValue(),
                getX() + 3 + 2 + FontUtil.getWidth(size, iconText),
