...
public class TickShiftIndicator extends Module {
    public static TickShiftIndicator INSTANCE;
    private boolean lastMouseStatus = false;
    private double lastMouseX = -1;
    private double lastMouseY = -1;

    public TickShiftIndicator() {
        super("TickShiftIndicator", Category.HUD);
        INSTANCE = this;
        setX(0);
        setY(0);
        setWidth(0);
    ...
    public Option<String> text = addOption(new TextOption("Text", "{tickshift_used}% | {tickshift_saved} / {tickshift_max}"));
    ...
    public void onDrawSkia(DrawContext context, float tickDelta) {
        if (!blur.getValue() || (!TickShift.INSTANCE.getStatus()) && !(mc.currentScreen instanceof HudEditorScreen)) {
            return ;
        }
    ...
    public void onDraw2D(DrawContext context, float tickDelta) {
        if (isNull() || (!TickShift.INSTANCE.getStatus()) && !(mc.currentScreen instanceof HudEditorScreen)) {
            return ;
        }
