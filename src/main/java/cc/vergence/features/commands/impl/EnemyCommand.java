...
public class EnemyCommand extends Command {
	...
	public void runCommand(String[] parameters) {
		...
		if (parameters[0].equals("reset")) {
			...
			MessageManager.newMessage("Vergence", Vergence.TEXT.get("COMMANDS.MESSAGE.ENEMY.RESET"));
			return;
		}
		if (parameters[0].equals("list")) {
			if (Vergence.ENEMY.enemyList.isEmpty()) {
				MessageManager.newMessage("Vergence", Vergence.TEXT.get("COMMANDS.MESSAGE.ENEMY.EMPTY"));
				return ;
			}
		...
		if (parameters[0].equals("add")) {
			if (parameters.length == 2) {
				if (Vergence.FRIEND.isFriend(parameters[1])) {
					MessageManager.newMessage("Vergence", Vergence.TEXT.get("COMMANDS.MESSAGE.ENEMY.IS_FRIEND"));
					return;
				}
				Vergence.ENEMY.addEnemy(parameters[1]);
				MessageManager.newMessage("Vergence", Vergence.TEXT.get("COMMANDS.MESSAGE.ENEMY.ADD"));
				return;
			}
			sendUsage();
			return;
			...
			if (parameters.length == 2) {
				...
				MessageManager.newMessage("Vergence", Vergence.TEXT.get("COMMANDS.MESSAGE.ENEMY.REMOVE"));
				return;
			}
			sendUsage();
			return;
